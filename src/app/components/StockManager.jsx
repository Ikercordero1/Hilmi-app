    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-6">
      ...
      {alertCount > 0 && (
        ...
        >
          ...
          <svg className="w-6 h-6 text-teal-500 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-cyan-800 text-sm font-medium">Cargando inventario...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm">
          <svg className="w-16 h-16 text-teal-200 mx-auto mb-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
          </svg>
          <p className="text-gray-500 text-base font-medium">
            {search ? "No encontramos items que coincidan con tu búsqueda." : "El inventario está vacío."}
          </p>
        </div>
      ) : (
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-teal-50/50 text-teal-800 text-xs uppercase font-bold tracking-wider">
                    <tr>
                      ...
                      <th className="py-4 px-6 text-center hidden sm:table-cell">Mínimo</th>
                      <th className="py-4 px-6 text-center hidden md:table-cell">Precio</th>
                      <th className="py-4 px-6 text-center rounded-tr-2xl">Acciones</th>
                    </tr>
                  </thead>
                    ...
                    {catItems.map((item) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-center text-gray-500 font-medium hidden md:table-cell">
                          {item.price > 0 ? `$${parseFloat(item.price).toFixed(2)}` : "—"}
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            >
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.89 1.112l-2.83.793c-.5.14-.94-.3-.8-.8l.793-2.83a4.5 4.5 0 011.112-1.89l13.438-13.438zM16.862 4.487L19.5 7.125" />
                              </svg>
                            </button>
                            <button
                              onClick={() => {
            ...
            {(modal === "create" || modal === "edit") && (
              <>
                <h2 className="text-xl font-extrabold text-teal-900 mb-6 flex items-center gap-2">
                  <svg className="w-6 h-6 text-teal-600" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                    {modal === "create" 
                      ? <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                      : <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.89 1.113l-3.4.952c-.5.14-.94-.3-.8-.8l.953-3.4a4.5 4.5 0 011.112-1.89l10.305-10.305z" />
                    }
                  </svg>
                ...
                {error && <p className="text-red-500 text-xs font-medium bg-red-50 p-2 rounded-lg mb-4">{error}</p>}
            ...
            {modal === "adjust" && activeItem && (
              <>
                ...
                <div className="bg-teal-50 rounded-xl p-3 mb-5 border border-teal-100">
                  <p className="text-sm font-semibold text-teal-800">{activeItem.name}</p>
                  <p className="text-xs text-teal-600 mt-0.5">
                    Stock actual:{" "}
                    <strong className={isLow(activeItem) ? "text-red-600" : "text-teal-700"}>
                ...
                <div className="flex p-1 bg-gray-100 rounded-xl mb-5">
                  ...
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 12h-15" /></svg>
                    Salida
                  </button>
                </div>
                ...
                {adjustment && parseFloat(adjustment) > 0 && (
                  <div className="flex items-center justify-between bg-teal-50/50 border border-teal-100 rounded-xl px-4 py-3 mb-6">
                    <span className="text-sm text-teal-700 font-medium">Nuevo total:</span>
                    <span className="text-lg font-black text-teal-900">
                      {fmtQty(
                        adjustType === "add"
                          ? parseFloat(activeItem.quantity) + parseFloat(adjustment)
                          : parseFloat(activeItem.quantity) - parseFloat(adjustment),
                      )}{" "}
                      <span className="text-sm font-semibold text-teal-700">{activeItem.unit}</span>
                    </span>
                  </div>
                )}
            ...
            {modal === "confirmDelete" && activeItem && (
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mb-8">
                  Se borrará <strong className="text-gray-800">{activeItem.name}</strong> del sistema. Esta acción no se puede deshacer.
                </p>
                <div className="flex gap-3">
                  <SGhost onClick={closeModal}>Cancelar</SGhost>
                  <button
