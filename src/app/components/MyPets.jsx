            ...
            {form.file ? (
              ...
              <>
                <p className="text-sm text-neutral-600 dark:text-neutral-400">
                  Sube una foto del carnet físico
                </p>
                <p className="text-xs text-neutral-400 mt-0.5">
                  PNG, JPG o PDF · máx. 5 MB
                </p>
              </>
            )}
          </button>
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-xs text-neutral-300 mt-1">
            Usa el botón de arriba para añadir el historial previo
          </p>
        </div>
      ) : (
        <ul className="pl-3">
...
function PetCard({ pet, isOpen, onToggle, onVaccineAdd }) {
  ...
  return (
    <div
      className={`bg-white dark:bg-neutral-900 rounded-xl border transition-all duration-200
      ${
        isOpen
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/client/pets?cedula=${encodeURIComponent(trimmed)}`,
      );
      ...
      if (!data.pets || data.pets.length === 0) {
        setError(
          "No se encontraron mascotas con esa cédula. Verifica el número o contacta a la clínica.",
        );
        return;
      }
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 w-full max-w-sm p-8 text-center">
        ...
        <p className="text-sm text-neutral-500 mb-6">
          Ingresa tu número de cédula para ver las mascotas asociadas a tu
          perfil
        </p>
        ...
        <p className="text-xs text-neutral-400 mt-4">
          ¿No encuentras tus mascotas? Contacta a la clínica para que las
          asocien a tu perfil.
        </p>
      </div>
    </div>
...
export default function MisMascotas() {
  ...
  const handleVaccineSave = async (petId, vaccineData) => {
    ...
    const res = await fetch("/api/client/vaccines", {
      method: "POST",
      body: formData,
    });
      ...
      prev.map((p) =>
        p.id === petId
          ? {
              ...
              vaccines: [
                ...p.vaccines,
                { ...saved, note: saved.note ?? "Autodeclarada" },
              ],
            }
          : p,
      ),
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">
            Mostrando mascotas de la cédula{" "}
